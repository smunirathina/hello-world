...
# ── 4. Top CPU hogs ─────────────────────────────────────────
Section "Top 5 CPU Hogs (right now)"
...
# ── 6. Disk I/O (via Task Manager counters) ─────────────────
...
try {
    Get-PhysicalDisk | ForEach-Object {
        ...
        if ($type -eq "HDD") { WARN "HDD detected — much slower than SSD; consider upgrading" }
...
# ── 8. Temperature (if HWiNFO/OpenHardwareMonitor WMI available) ──
...
try {
    ...
    if ($temps) {
        ...
        elseif ($maxTemp -ge 80) { WARN "CPU running hot — check thermal paste / fans" }
        else                     { OK   "Temperature normal" }
    } else {
        Write-Host "  No sensor data found." -ForegroundColor DarkGray
        Write-Host "  Install OpenHardwareMonitor and enable WMI support to see temps." -ForegroundColor DarkGray
    }
} catch {
    Write-Host "  Temperature check skipped (OpenHardwareMonitor not running)." -ForegroundColor DarkGray
    Write-Host "  Download: https://openhardwaremonitor.org" -ForegroundColor DarkGray
}
...
if ($startupItems.Count -gt 10) { WARN "$($startupItems.Count) startup items — many apps auto-launch" }
elseif ($startupItems.Count -gt 0) { OK  "$($startupItems.Count) startup items found" }
else { OK "No registry startup items found" }

# ── 10. Page file (virtual memory) ──────────────────────────
...
Get-CimInstance Win32_PageFileUsage | ForEach-Object {
    ...
    elseif ($pct -ge 40) { WARN "Page file in moderate use — your PC is paging to disk" }
    else                 { OK   "Page file usage low" }
}

# ── Summary ──────────────────────────────────────────────────
Write-Host "`n$sep" -ForegroundColor Cyan
Write-Host "  Done! Review any [!!] or [XX] items above." -ForegroundColor Yellow
Write-Host $sep -ForegroundColor Cyan
Write-Host ""
